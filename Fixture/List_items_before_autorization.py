...
class BeforeAutorizationHelper:
    ...
    def Test_list_of_all_items(self, TEXT):
        ...
        for i in range(0, lenght_Items):
            ...
            if Lenght_Objects != 0:
                number_tests = self.Test_test_list(TEXT)   # функция выбор теста по порядку
    ...
    # нажатие на урок/тест по порядку, возвращает кол-во уроков/тестов в предмете
    def Test_test_list(self, TEXT):
        driver = self.app.driver
        ...
        n = 0   # Кол-во уроков/тестов для которых есть доступ посте того как пользователь авторизуется
